...
import com.uramp.urcap.logger.impl.EcLogLevel;
import com.uramp.urcap.logger.impl.EcLogger;
...
public class RTDEClient {
    ...
    public synchronized boolean connect(String host, int port, int timeout) {
        EcLogger.out(EcLogLevel.DEBUG).println("Connecting to RTDE " + host + ":" + port);
        if (this.socket != null) {
        ...
        try {
            ...
            this.outputStream =
                    new RTDEOutputStream(new DataOutputStream(socket.getOutputStream()));
            ...
            EcLogger.out(EcLogLevel.ERROR).println("Can't connect to RTDE." + e);
            disconnect();
            return false;
        }
    }
    ...
    private synchronized void receive() {
        if (socket == null) {
            EcLogger.out(EcLogLevel.ERROR).println("Can't read. Connection is closed");
            return;
        }
        try {
            ...
            EcLogger.out(EcLogLevel.ERROR).println("Unexpected exception" + e);
            disconnect();
        }
    }
    ...
    public synchronized boolean disconnect() {
        ...
        EcLogger.out(EcLogLevel.DEBUG).println("Disconnected");
        return true;
    }
    ...
    public synchronized void setupOutputsV2(double outputFrequency, RTDEOutputParam... outputs)
            throws IOException {
        if (outputs.length == 0) {
            return;
        }
    ...
    private void onControlPackageInputsResponse(int size) throws IOException {
        ...
        if (id != 0) {
            ...
            handler.onSetupInputsResponse(id, null,
                    unsupportedParams.toArray(new RTDEInputParam[] {}));
        }
        EcLogger.out(EcLogLevel.DEBUG).format("VALUES {}", values);
    }
    ...
    private void recepyOutputResponse(int stringSize) throws IOException {
        ...
        EcLogger.out(EcLogLevel.DEBUG).println("VALUES " + values);
    }
    ...
    private void onDataPackage(int size) throws IOException {
        ...
        for (int i = 0; i < this.outParameters.length; i++) {
            ...
            RTDEDataType type = this.outParameters[i].type;

            switch (type) {
                case VECTOR6D:
                    this.handler.onData(var, type, this.inputStream.readVector6Double());
                    break;
                case VECTOR3D:
                    this.handler.onData(var, type, this.inputStream.readVector3Double());
                    break;
                case VECTOR6INT32:
                    this.handler.onData(var, type, this.inputStream.readVector6Int32());
                    break;
                case VECTOR6UINT32:
                    this.handler.onData(var, type, this.inputStream.readVector6UInt32());
                    break;
                case DOUBLE:
                    this.handler.onData(var, type, this.inputStream.readDouble());
                    break;
                case UINT64:
                    this.handler.onData(var, type, this.inputStream.readUInt64());
                    break;
                case UINT32:
                    this.handler.onData(var, type, this.inputStream.readUInt32());
                    break;
                case INT32:
                    this.handler.onData(var, type, this.inputStream.readInt32());
                    break;
                case BOOL:
                    this.handler.onData(var, type, this.inputStream.readUInt8() != 0);
                    break;
                case UINT8:
                    this.handler.onData(var, type, this.inputStream.readUInt8());
                    break;
            }
        }
    }
    ...
    private void onUnsupportedPackage(int size, int type) throws IOException {
        ...
        this.inputStream.readFully(data);
        EcLogger.out(EcLogLevel.WARNING).format("Unsupported package type {} size={} value={}",
                type, size,
    ...
    private static void closeSilently(Closeable closeable) {
        try {
            ...
            EcLogger.out(EcLogLevel.WARNING).println(e.getMessage() + e);
        }
    }
